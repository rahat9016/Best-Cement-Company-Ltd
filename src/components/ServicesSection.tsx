      ...
      <div className="container">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {services.map((service) => (
            <div
              key={service.title}
              className="bg-light rounded-lg p-5 sm:p-7 group hover:bg-dark hover:-translate-y-2 transition-all duration-300 cursor-pointer"
            >
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-lg bg-primary/10 flex items-center justify-center mb-4 sm:mb-5 group-hover:bg-primary/20 transition-colors">
                {service.icon}
              </div>
              <h3 className="font-heading font-bold text-sm sm:text-lg text-text group-hover:text-white mb-2 sm:mb-3 transition-colors">
                {service.title}
              </h3>
              <p className="text-text-muted text-xs sm:text-sm leading-relaxed group-hover:text-white/50 transition-colors">
                {service.description}
              </p>
            </div>
          ))}
